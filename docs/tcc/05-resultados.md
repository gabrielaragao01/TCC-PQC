...
Os dados revelam uma diferença de aproximadamente 2.206 vezes entre o RSA-2048 e o ML-DSA-44 na geração de chaves, e de cerca de 5.888 vezes entre o RSA-2048 e o Kyber512 (razões calculadas a partir das médias não arredondadas). Essa disparidade decorre da própria natureza matemática dos algoritmos. A geração de chaves RSA exige a obtenção de dois números primos de grande magnitude, processo que envolve testes de primalidade probabilísticos (como Miller-Rabin) cujo custo cresce de forma superlinear com o tamanho da chave. Os algoritmos baseados em reticulados, em contraste, geram chaves a partir de operações matriciais sobre polinômios em anéis cíclicos, com complexidade dominada por transformadas teórico-numéricas (NTT) executadas em tempo $O(n \log n)$ (NIST, 2024a, 2024b).
...
Na prática, substituir o RS256 pelo ML-DSA-44 economiza mais de 1,4 ms de assinatura por login. Em servidores reais com I/O assíncrono e múltiplos workers, esse ganho não reduz o tempo de resposta de uma requisição isolada — dominado pela latência de rede e pelas demais camadas —, mas eleva a vazão (requisições por unidade de tempo) para a mesma alocação de CPU. O resultado contradiz o senso comum de que a criptografia pós-quântica seria computacionalmente mais lenta que a clássica: o sobrecusto percebido decorre do tamanho de chaves e tokens (Seção 5.4), e não do custo das operações criptográficas em si.
...
A comparação entre o `jwt_sign` (1,629 ms) e o `raw_rsa_sign` (89,323 ms) merece atenção especial. À primeira vista, a diferença sugeriria que a camada de serviço seria 54 vezes mais rápida que a primitiva — interpretação evidentemente equivocada. A explicação reside na metodologia: a primitiva raw mede o custo total de uma operação RSA "do zero", incluindo a desserialização da chave privada DER (`load_der_private_key`) a cada iteração — processo que domina o tempo medido, da ordem de 88 ms por chamada no ambiente experimental, ante menos de 1 ms da operação de assinatura RSA propriamente dita. No nível de serviço, a chave RSA é parseada uma única vez na inicialização do `ClassicalAuthService` e mantida em memória como objeto Python pré-carregado, eliminando esse custo recorrente. A medição em camada de serviço, portanto, reflete adequadamente o regime permanente de um servidor de produção, no qual chaves não são parseadas novamente a cada requisição. Cabe notar que a primitiva `raw_rsa_*` utiliza o preenchimento RSA-PSS, ao passo que o `jwt_sign` adota o RS256/PKCS#1 v1.5; conforme registrado na Seção 4.3.1, essa diferença de preenchimento não afeta materialmente o custo da assinatura e, portanto, não contribui para a disparidade observada, integralmente explicada pela nova serialização da chave. Já na verificação, observa-se o padrão inverso — a primitiva isolada (0,113 ms, RSA-PSS via `cryptography`) é mais lenta que a camada de serviço (0,058 ms, PKCS#1 v1.5 via PyJWT); nessa escala de dezenas de microssegundos, a diferença reflete os caminhos de código e o esquema de preenchimento distintos das duas bibliotecas, e não um custo intrínseco da verificação, permanecendo ambos os valores desprezíveis em termos absolutos.
...
A análise consolidada deste trade-off é que o principal obstáculo à adoção do ML-DSA-44 em autenticação web não reside no custo computacional — favorável ao algoritmo pós-quântico, como demonstrado nas seções anteriores — mas no custo de transmissão, o que desloca o foco da migração para o redimensionamento da infraestrutura de rede.
...
Três fatores explicam o resultado. Primeiro, o IPC elevado dos processadores Apple Silicon, com execução fora de ordem agressiva e hierarquia de cache de baixa latência, particularmente eficaz para os padrões de acesso localizado das operações de NTT. Segundo, as otimizações ARM64 do liboqs 0.15.0 — instruções NEON SIMD de 128 bits e aceleração de AES via extensão `FEAT_AES` —, ausentes na implementação de referência do NIST. Terceiro, o overhead da camada CFFI (Python-C), estimado em 1 a 5 microssegundos por chamada, desprezível frente ao custo das operações analisadas. A paridade com o AVX2 não é alcançada no Kyber512 porque o algoritmo depende fortemente de NTT com operações de 32 bits empacotadas, em que a largura SIMD de 256 bits do AVX2 supera a de 128 bits do NEON; o ML-DSA-44, com padrões de acesso mais homogêneos, não se beneficia tanto dessa largura.

Cabe destacar que a comparação com as referências NIST não pretende equiparar plataformas distintas, e sim estabelecer que os resultados obtidos são da ordem de grandeza esperada para implementações otimizadas dos algoritmos avaliados. Como o overhead da camada Python/CFFI é simétrico entre algoritmos clássicos e pós-quânticos — ambos sofrem o mesmo custo fixo por chamada —, o speedup de cerca de 8,0 vezes do ML-DSA-44 sobre o RS256 na assinatura, no nível de serviço, é genuíno, e não um artefato da camada de linguagem.
...
Além das especificações do NIST, os resultados podem ser confrontados com a literatura empírica recente, ressalvadas as diferenças de plataforma e de camada de medição. Demir, Gorgulu e Onbasli (2026) reportam, em x86_64, um handshake completo de TLS 1.3 com ML-KEM-512 e ML-DSA-44 em cerca de 2,7 ms, valor próximo ao do P-256 clássico (2,2 ms). O presente trabalho mede a camada criptográfica isoladamente, sem o handshake de transporte: a soma das operações pós-quânticas exercitadas — assinatura, verificação e round-trip de KEM — permanece na casa das centenas de microssegundos, uma fração pequena daquele handshake. Os dois resultados são, portanto, coerentes e complementares: confirmam, em camadas distintas, que o custo computacional da PQC neste nível de segurança é equiparável ao clássico, e localizam o overhead remanescente fora da operação criptográfica em si.

No mesmo sentido, Abbasi et al. (2025) observam que a geração de chaves do RSA-3072 é ordens de magnitude mais lenta que a de qualquer alternativa pós-quântica — direção idêntica à medida na Seção 5.1.1, em que o RSA-2048 keygen é cerca de 2.206 vezes mais lento que o ML-DSA-44 e 5.888 vezes mais lento que o Kyber512. A comparação de valores absolutos exige cautela, pois Demir et al. e Abbasi et al. medem em x86_64 com implementações em C, ao passo que este trabalho usa ARM64 com camada Python (Seções 2.6 e 4.6.2); ainda assim, a concordância direcional reforça a validade externa dos achados.

## 5.7 Análise do Modo Híbrido

O modo híbrido executa simultaneamente o algoritmo clássico e o pós-quântico em uma mesma requisição, retornando dois tokens distintos no login. Destina-se a apoiar a migração incremental, permitindo que clientes legados continuem a verificar tokens RS256 enquanto clientes atualizados usam tokens ML-DSA-44, sem transição abrupta da infraestrutura. A Tabela 5.10 compara suas latências com as dos modos isolados.
...
O achado central é que sobrepor as duas operações em uma mesma requisição não introduz custo além da soma de suas latências individuais — os desvios observados (entre 0,5% e 4,5%) ficam dentro do coeficiente de variação inter-execução da Seção 5.5 e são atribuíveis a variabilidade ambiental, não a um efeito de composição. A adoção do híbrido como solução transitória é, portanto, viável sem custo computacional perceptível; seu custo dominante está no volume transmitido — o response inclui ambos os tokens, cerca de 3,8 KB contra 0,5 KB do modo clássico —, o que reitera o tamanho de payload como variável crítica da migração (Seção 5.4).

A leitura agregada dos resultados deste capítulo permite caracterizar a adoção do ML-DSA-44 e do Kyber512 em sistemas reais de autenticação web como tecnicamente viável e, em diversas dimensões, vantajosa frente à alternativa clássica. A vantagem em latência de assinatura, a estabilidade reprodutível das medições e a paridade com implementações otimizadas em C sustentam essa avaliação. Em contrapartida, o aumento expressivo do tamanho dos tokens e das chaves constitui o principal obstáculo prático à migração, especialmente em arquiteturas sensíveis a tráfego de rede e em ambientes de infraestrutura previamente dimensionados para tokens JWT clássicos.
